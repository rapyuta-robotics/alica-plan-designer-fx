            ...
            for (String domainName : split) {
                if (!domainName.isEmpty()) {
                    ...
                    if (conf.loadFromDisk()) {
                        ...
                        LOG.error("Could not load the config '" + conf.getName() + "' from disk.");
                        generateDefaultConfiguration();
                    }
                }
            }
...
//            String pluginPath = currentRelativePath.toAbsolutePath().toString() + "/alica-plan-designer-fx-default-plugin/target";
            ...
            File planFile = new File(String.valueOf(xFileChooser.getSelectedFile())+ "/plans");
            if (!planFile.exists()) planFile.mkdirs();

            File taskFile = new File(String.valueOf(xFileChooser.getSelectedFile())+ "/tasks");
            if (!taskFile.exists()) taskFile.mkdirs();

            File roleFile = new File(String.valueOf(xFileChooser.getSelectedFile())+ "/roles");
            if (!roleFile.exists()) roleFile.mkdirs();

            File genFile = new File(String.valueOf(xFileChooser.getSelectedFile())+ "/autogen");
